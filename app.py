from typing import Dict  # Ajoutez cette ligne en haut du fichier
...
def setup_and_run_pagerank():
    """Configure les permissions et exécute le PageRank"""
    try:
        # Rendre les scripts exécutables
        os.chmod('run_pagerank.sh', stat.S_IRWXU)
        os.chmod('mapper.py', stat.S_IRWXU)
        os.chmod('reducer.py', stat.S_IRWXU)
        
        # Exécuter le PageRank
...
def compare_with_networkx(pagerank_results: Dict[str, float]):
    """Compare les résultats avec NetworkX"""
    G = nx.DiGraph()
    with open("input.txt") as f:
        for line in f:
            nodes = line.strip().split()
            if len(nodes) > 1:
                G.add_edges_from((nodes[0], target) for target in nodes[1:])
    
    nx_pagerank = nx.pagerank(G, alpha=DAMPING)
    
    # Calcul des différences
    comparison = {
        'rmse': np.sqrt(np.mean([
            (nx_pagerank[node] - pagerank_results.get(node, 0))**2
            for node in nx_pagerank
        ])),
        'correlation': np.corrcoef(
            list(nx_pagerank.values()),
            [pagerank_results.get(node, 0) for node in nx_pagerank]
        )[0, 1]
    }
        ...
        if st.button("📈 Analyser Graphe"):
            if Path("input.txt").exists():
                with st.spinner("Analyse en cours..."):
                    ...
                    # Créez un conteneur pour les stats
...
# Section des résultats - Corrigée
st.header("📋 Résultats")

tabs = st.tabs(["PageRank Final", "Évolution", "Visualisation", "Métriques Avancées"])

with tabs[0]:
    # Affichage du PageRank final - Version corrigée
    st.subheader("🏆 Résultats finaux")
    
    try:
        # Trouver le dernier fichier d'itération
        iteration_files = sorted(Path(".").glob("iteration*.txt"), 
                               key=lambda x: int(x.stem.replace("iteration", "")))
        
        if iteration_files:
            latest_file = max(iteration_files, key=lambda x: int(x.stem.replace("iteration", "")))
                ...
                if len(results) > 0:
                    ...
                    for bar, rank in zip(bars, ranks):
                        ...
                        ax.text(bar.get_x() + bar.get_width()/2., height + 0.001,
                               f'{rank:.4f}', ha='center', va='bottom')
                    
                    plt.tight_layout()
                    st.pyplot(fig)
            else:
                st.warning("Aucun résultat valide à afficher")
        else:
            st.warning("Aucun fichier d'itération trouvé")
    except Exception as e:
        st.error(f"Erreur lors de la lecture des résultats: {str(e)}")

with tabs[1]:
    # Évolution du PageRank - Version corrigée
    ...
    try:
        ...
        if len(iteration_files) > 1:
            ...
            if evolution_data:
                ...
                convergence_df = pd.DataFrame.from_dict(evolution_data, orient='index')
                convergence_df = convergence_df.fillna(0).sort_index()
                st.dataframe(convergence_df, use_container_width=True)
            else:
                st.warning("Aucune donnée d'évolution à afficher")
        else:
            st.warning("Pas assez de fichiers d'itération pour afficher l'évolution")
    except Exception as e:
        st.error(f"Erreur lors de la lecture des données d'évolution: {str(e)}")
